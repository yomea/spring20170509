...
public class SimpleTriggerExample {
  ...
  public void run() throws Exception {
    ...
    /**
     * 所有的作业都加入到了scheduler中，只用调用了start才开始工作
     */
    sched.start();

    log.info("------- Started Scheduler -----------------");

    // jobs can also be scheduled after start() has been called...作业也可以在调度器启动之后
    ...
    //添加一个没有触发器的作业，这个作业将被休眠，知道这个作业被调度，或者Scheduler.triggerJob()被调用 
   //而这个job必须是持久化的
    sched.addJob(job, true);

    log.info("'Manually' triggering job8...");
    //JobKey.jobkey(String name, String group);封装成一个JobKey对象
    sched.triggerJob(jobKey("job8", "group1"));//触发job8
    ...
    //移除trigger.getKey的触发器，重新使用trigger触发器
