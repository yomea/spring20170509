 ...
 * @author Bill Kratzer
 */
@PersistJobDataAfterExecution//在执行完成后能够更新JobDataMap里的数据
@DisallowConcurrentExecution//表示要进行同步，同时只能有一个这个作业定义（JobDetail）的作业实例(名字相同的实例)在执行
//名字相同的作业定义实例只能同时执行一个，比如一个MyJob作业类，然后基于它定义了a，b两个JobDetail，a和b是可以同时执行的，如果另一个触发器触发a
//的JobDetail,那么只能在同一时间执行其中的一个实例
